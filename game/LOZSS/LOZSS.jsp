...
%>
	...
	<div id="maindiv">
			...
			<table class="gm">
				<tr>
					<td class="picture" rowspan="2">
						<img style="height:720px" src="../img/background/LOZSS.PNG">
					</td>
					<td><%= request.getRequestURL().substring(27,32) %></td>
				</tr>
				<tr>
					<td></td>
				</tr>
		...
		bbs.setUrl((String)request.getRequestURL().substring(27,32));
		bbs.setUrl2((String)request.getRequestURL().substring(27,32)); 
		...
		<tr>
		<td><a href="LOZSS.jsp?pageNumber=<%if(pageNumber/10==0){%><%=pageNumber=01%><% }else{ %><%=pageNumber -10%><%}%>">이전</a></td>
		<td><a href="LOZSS.jsp?pageNumber=<%= pageNumber/10%>1"><%= pageNumber/10%>1</a></td>
		<%if((bbsDAO.getPage()-1)>pageNumber*10){ %>
		<td><a href="LOZSS.jsp?pageNumber=<%= pageNumber/10%>2"><%= pageNumber/10%>2</a></td>
		<% }if((bbsDAO.getPage()-1)>pageNumber*20){%>
		<td><a href="LOZSS.jsp?pageNumber=<%= pageNumber/10%>3"><%= pageNumber/10%>3</a></td>
		<% }if((bbsDAO.getPage()-1)>pageNumber*30){%>
		<td><a href="LOZSS.jsp?pageNumber=<%= pageNumber/10%>4"><%= pageNumber/10%>4</a></td>
		<% }if((bbsDAO.getPage()-1)>pageNumber*40){%>
		<td><a href="LOZSS.jsp?pageNumber=<%= pageNumber/10%>5"><%= pageNumber/10%>5</a></td>
		<% }if((bbsDAO.getPage()-1)>pageNumber*50){%>
		<td><a href="LOZSS.jsp?pageNumber=<%= pageNumber/10%>6"><%= pageNumber/10%>6</a></td>
		<% }if((bbsDAO.getPage()-1)>pageNumber*60){%>
		<td><a href="LOZSS.jsp?pageNumber=<%= pageNumber/10%>7"><%= pageNumber/10%>7</a></td>
		<% }if((bbsDAO.getPage()-1)>pageNumber*70){%>
		<td><a href="LOZSS.jsp?pageNumber=<%= pageNumber/10%>8"><%= pageNumber/10%>8</a></td>
		<% }if((bbsDAO.getPage()-1)>pageNumber*80){%>
		<td><a href="LOZSS.jsp?pageNumber=<%= pageNumber/10%>9"><%= pageNumber/10%>9</a></td>
		<% }if((bbsDAO.getPage()-1)>pageNumber*90){%>
		<td><a href="LOZSS.jsp?pageNumber=<%= pageNumber/10%>0"><%= pageNumber/10+1%>0</a></td>
		<% }if((bbsDAO.getPage()-1)>pageNumber*100){%>
		<td><a href="LOZSS.jsp?pageNumber=<%=pageNumber +10%>">다음</a></td>
		<%} %></tr>
		</table>
